...
;;(d) Create a gettiming list that takes desired length as a paramter
(define (gettiminglist listSize)
  (let ( (random-number (+ 1 (random (length grocerylist ))) ))
    (if (= listSize 0) '() (cons random-number ( gettiminglist (- listSize 1))) )))

;;(e) The helper function that calls the gettiminglist function passing it an argument of 20000 and returned list to the subtotal function
(define (runtime)
